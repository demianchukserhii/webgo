                ...
                function g() {
                    ...
                    try {
                        ...
                        m.iframeTarget || q.appendTo("body"),
                            r.attachEvent ? r.attachEvent("onload", h) : r.addEventListener("load", h, !1), setTimeout(b, 15);
                        try {
                            x.submit()
                        } catch (a) {
...
$(window).on("load", function() {
    ...
    if ($(".mobile .slides:not(.scroll):not(.simplifiedMobile), .slides.desktopSwipe").swipe({
            ...
            $(window).on("load resize ready", function() {
                ...
                if ((window.isMobile || $(document).width() < 767) && f) $(d).addClass("hidden"), $(e).removeClass("hidden");
                else {
    ...
    $(x).length > 0 && $(x).each(function(a, b) {
        ...
        if (window.sliderStatus && $html.removeClassByPrefix("slider_" + d).addClass("slider_" + d + "_" + e), c.hasClass("autoplay")) {
            ...
            var b = $(this),
                c = b.children(".selected"),
                d = c.nextOrFirst("li"),
                e = d.index(),
                f = b.data("slider-id"),
                g = $('.controller[data-slider-id="' + f + '"]'),
                h = b.hasClass("animated"),
                i = a.target;
            "cancel" != $(i).data("slider-event") && (c.removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function() {
                $(this).removeClass("hide")
            }), d.removeClass("hide").addClass("selected"), window.sliderStatus && $html.removeClassByPrefix("slider_" + f).addClass("slider_" + f + "_" + e), h && (b.addClass("animateOnEvent"), b.find("li").removeClassByPrefix("ae-").removeClass("do"), b.find(".selected").each(function(a) {
                $(this).removeClassByPrefix("ae-").removeClass("do").addClass("ae-" + (a + 1)).addClass("do")
            }), $(window).scroll()), f && g.length > 0 && (g.children(".selected").removeClass("selected"), g.children("li:eq(" + e + ")").addClass("selected")))
        })
    });
    var y = $(".controller");
    if (y.length > 0) {
        ...
        y.on("click", z, function() {
            var a = $(this),
                ...
                d = $(b.find(z)).index(a),
                e = b.data("slider-id"),
                f = $('.slider[data-slider-id="' + e + '"]'),
                g = f.hasClass("animated");
            a.hasClass("selected") || (c.removeClass("selected"), a.addClass("selected"), f.children(".selected").removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function() {
                $(this).removeClass("hide")
            }), f.children("li").eq(d).removeClass("hide").addClass("selected"), window.sliderStatus && $html.removeClassByPrefix("slider_" + sliderID).addClass("slider_" + sliderID + "_" + d)), g && (f.addClass("animateOnEvent"), f.find(">li").removeClassByPrefix("ae-").removeClass("do"), f.find(".selected").each(function(a) {
                $(this).removeClassByPrefix("ae-").removeClass("do").addClass("ae-" + (a + 1)).addClass("do")
            }), $(window).scroll())
        })
    }
    $("[data-slider-action]").click(function() {
        if ($(this).data("slider-id")) {
            var a, b, c = $(this),
                ...
                h = g.data("controller-selector") ? g.data("controller-selector") : "li",
                i = f.find(".selected"),
                j = f.hasClass("animated");
            "next" === e ? a = i.nextOrFirst("li") : "prev" === e ? a = i.prevOrLast("li") : (parseInt(e) || 0 === e) && (b = parseInt(e), a = f.find(">li:eq(" + b + ")")), b = a.index(), i.removeClass("selected"), a.removeClass("hide").addClass("selected"), window.sliderStatus && $html.removeClassByPrefix("slider_" + d).addClass("slider_" + d + "_" + b), j && (f.addClass("animateOnEvent"), f.find("li").removeClassByPrefix("ae-").removeClass("do"), f.find(".selected").each(function(a) {
                $(this).removeClassByPrefix("ae-").removeClass("do").addClass("ae-" + (a + 1)).addClass("do")
            }), $(window).scroll()), d && g.length > 0 && (g.find(".selected").removeClass("selected").addClass("hide").one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function() {
                c.removeClass("hide")
            }), g.find(h).eq(b).addClass("selected"))
        }
